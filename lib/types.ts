...
export type DiaryEntryType = 'care' | 'mood' | 'illness' | 'revive' | 'hatch' | 'event' | 'couple' | 'discovery';
export interface DiaryEntry { id: string; date: string; text: string; favorite?: boolean; type?: DiaryEntryType; createdAt?: number; }
export interface MoodMemory { id: string; petId: string; type: 'happy' | 'lonely' | 'sick' | 'first-bath' | 'first-level-up' | 'first-sickness' | 'first-event' | 'first-egg' | 'first-revive'; text: string; createdAt: number; }
export type EggStatus = 'incubating' | 'ready' | 'hatched';
export interface PetEgg { id: string; parentIds: string[]; createdAt: number; hatchAt: number; rarityChance: number; inheritedTraits: Partial<{ species: string; personality: PetPersonality; favoriteFood: string; favoriteToy: string; variantId: string }>; status: EggStatus; hatchedPetId?: string; }
export interface PetVariant { variantId: string; displayName: string; baseAnimalId: string; rarity: Rarity; colorTheme: string; unlockSource: 'breeding' | 'event' | 'shop' | 'achievement' | 'default'; limitedEventTag?: EventTag; }
export interface SaveRepairLog { id: string; createdAt: number; repairs: string[]; }
export interface AntiCheatFlag { id: string; createdAt: number; code: string; message: string; }
export interface SaveMetadata { lastBackupAt: number | null; backupCount: number; repairLog: SaveRepairLog[]; antiCheatFlags: AntiCheatFlag[]; lastDeviceTime: number | null; }
export interface CloudSyncMetadata { enabled: boolean; autoSync: boolean; lastSyncedAt: number | null; deviceName: string; status: 'disabled' | 'idle' | 'syncing' | 'synced' | 'error' | 'conflict'; }
export interface NewsReadState { readIds: string[]; lastReadVersion: string | null; }
export interface TournamentState { weekId: string; gameOfTheDay: string; bestScores: Record<string, number>; playsToday: number; claimedRewardIds: string[]; }
export interface WishlistItem { itemId: string; pinned: boolean; addedAt: number; }
