...
export const safeParseSave = (raw: string | null): SaveData => {
  ...
  try {
    ...
    return withOfflineProgress({
      ...
      saveVersion: CURRENT_SAVE_VERSION,
      moodMemory: Array.isArray(parsed.moodMemory) ? parsed.moodMemory : [],
      skillPoints: Math.max(0, parsed.skillPoints ?? 0),
      unlockedSkills: Array.isArray(parsed.unlockedSkills) ? parsed.unlockedSkills : [],
      metadata: { ...base.metadata!, ...(parsed.metadata ?? {}), repairLog:Array.isArray(parsed.metadata?.repairLog) ? parsed.metadata.repairLog : [], antiCheatFlags:Array.isArray(parsed.metadata?.antiCheatFlags) ? parsed.metadata.antiCheatFlags : [] },
      eggs: Array.isArray(parsed.eggs) ? parsed.eggs : [],
      breedingCooldownUntil: parsed.breedingCooldownUntil ?? null,
      unlockedVariants: Array.isArray(parsed.unlockedVariants) ? parsed.unlockedVariants : [],
      activeVariantByPet: parsed.activeVariantByPet ?? {},
      reviveFreeUsed: parsed.reviveFreeUsed ?? false,
      news: { ...base.news!, ...(parsed.news ?? {}), readIds:Array.isArray(parsed.news?.readIds) ? parsed.news.readIds : [] },
      cloudSync: { ...base.cloudSync!, ...(parsed.cloudSync ?? {}) },
      eventCurrency: Math.max(0, parsed.eventCurrency ?? 0),
      gems: Math.max(0, parsed.gems ?? 0),
      tournament: { ...base.tournament!, ...(parsed.tournament ?? {}), bestScores:parsed.tournament?.bestScores ?? {}, claimedRewardIds:Array.isArray(parsed.tournament?.claimedRewardIds) ? parsed.tournament.claimedRewardIds : [] },
