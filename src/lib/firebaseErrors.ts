...
export function getFirebaseFriendlyError(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    'auth/network-request-failed': 'Connection unavailable. Your progress is safe locally.',
    ...
    unavailable: 'Connection unavailable. Your progress is safe locally.',
    'deadline-exceeded': 'Connection timed out. Please try again.',
    'failed-precondition': 'Firebase is not ready for this action.',
    cancelled: 'Connection was interrupted.',
    offline: 'You’re offline. This action will sync later.',
    'online-only': 'This feature needs internet.',
  ...
  if (typeof error === 'object' && error !== null && 'message' in error) {
    ...
    if (message.includes('offline')) return messages.offline;
    if (message.includes('needs internet')) return messages['online-only'];
    if (message.includes('permission')) return messages['permission-denied'];
    if (message.includes('unavailable') || message.includes('network') || message.includes('connection')) return messages.unavailable;
