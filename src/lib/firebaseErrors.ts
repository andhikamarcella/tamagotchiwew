...
export function getFirebaseFriendlyError(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    'auth/unauthorized-continue-uri': 'Password reset link blocked because the app URL is not authorized in Firebase. Add NEXT_PUBLIC_APP_URL with an authorized domain or add this domain in Firebase Authentication settings.',
    'auth/invalid-continue-uri': 'Password reset link URL is invalid. Check NEXT_PUBLIC_APP_URL in your deployment settings.',
    'auth/missing-continue-uri': 'Password reset link URL is missing. Check Firebase action-code settings.',
