...
export function useCreateRoom() {
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [createStep, setCreateStep] = useState<'idle' | 'starting' | 'timeout' | 'created'>('idle');
  ...
  const createRoom = useCallback(async (nickname: string, roomName: string) => {
    ...
    try {
      ...
      const roomId = await createDefaultRoomDocs(nextCode, cleanName, roomName, { uid: user.uid, photoURL: user.photoURL });
...
export function useJoinRoom() {
  ...
  const joinRoom = useCallback(async (nickname: string, inviteCode: string) => {
    ...
    try {
      ...
      const friendly = err instanceof Error && ['Invite code not found.', 'This room is closed.', 'This room is already full.'].includes(err.message) ? err.message : getFriendlyFirebaseError(err);
      setError(friendly);
      return null;
    } finally {
      setLoading(false);
