...
export default function CoupleModePage() {
  ...
  if (!network.isOnline) return <><TinyToast message={toast} /><Card className="bg-yellow-100"><h2 className="mb-2 text-sm">Couple Mode needs internet</h2><p className="mb-3 text-[10px] leading-relaxed">Reconnect to create or join rooms. Connection lost. Couple actions are paused.</p><Button onClick={() => showToast('This feature needs internet.')}>Reconnect to continue</Button></Card></>;
...
export function CreateRoomCard({ nickname, setRoomId, setCreatedInvite, showToast, isOnline = true }: { nickname: string; isOnline?: boolean; setRoomId: (id: string) => void; setCreatedInvite: (invite: CreatedInvite) => void; showToast: (m: string) => void }) {
  ...
  const handleCreate = async () => {
    if (!isOnline) { showToast('You’re offline. Connect to the internet to create an invite code.'); return; }
