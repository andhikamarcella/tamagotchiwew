...
Pixel Paws exposes `/manifest.json` with installable 192px/512px regular and maskable PNG icon endpoints generated deterministically at build/runtime, so the Git diff stays text-only. The root `/sw.js` service worker imports the existing conservative offline and optional Firebase Messaging handlers, so PWABuilder can detect one root-scoped worker without competing with authentication, Firestore, Couple Mode, or Vercel routing. Build and deploy the production app, verify `/manifest.json` and `/sw.js` return HTTP 200, then rescan the deployed URL in PWABuilder before selecting **Package For Stores**.
