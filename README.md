...
NEXT_PUBLIC_APP_URL=https://your-authorized-domain.example
```
...
NEXT_PUBLIC_AUTH_ACTION_URL=https://your-authorized-domain.example
```

12. Redeploy project setelah env diisi atau diubah.

> Password reset / verifikasi email: `NEXT_PUBLIC_APP_URL` harus memakai domain yang sudah ada di **Firebase Authentication > Settings > Authorized domains**. Jangan biarkan link reset memakai domain preview Vercel yang belum authorized, karena Firebase akan menolak email action link tersebut. Jika belum yakin domainnya authorized, app akan mengirim password reset dengan handler default Firebase agar email tetap terkirim.
